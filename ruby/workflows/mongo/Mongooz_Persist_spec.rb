...
describe Mongooz do
	describe "::Persist" do
		...
		describe "when included in a class" do
			it "defines an update function on an instance of your class" do
				expect(@pt.methods.include?(:update)).to be_true
			end
			it "defines an insert function on an instance of your class" do
				expect(@pt.methods.include?(:insert)).to be_true
			end
			it "doesn't define insert or update on the class itself" do
				expect(PersistTest.methods.include?(:update)).to_not be_true
				expect(PersistTest.methods.include?(:insert)).to_not be_true
			end
		end
		describe "insert" do
			it "throws an exception if you insert nil" do
				expect{@pt.insert(nil, :collection=>@test_collection)}.to raise_error
			end
			it "throws an exception with no args" do
				expect{@pt.insert}.to raise_error
			end
			it "returns a bson object when you insert something non-nil" do
				expect(@pt.insert({:name=>'inserted', :value=>987987}, :collection=>@test_collection)).to be_an_instance_of(BSON::ObjectId)
			end
		end
		describe "update" do
			it "throws exception if called with less than 2 args" do
				expect{@pt.update}.to raise_error
				expect{@pt.update('adf')}.to raise_error
			end
			it "throws exception if second arg is not a hash" do
				expect{@pt.update('adsf',nil)}.to raise_error
				expect{@pt.update('asdf',[])}.to raise_error
				expect{@pt.update('asdf','will barf')}.to raise_error
			end
			it "returns a hash with an 'n' key which is a Fixnum if args are valid" do
				err_hash=@pt.update('junk string',{}, :collection=>@test_collection)
			...
			it "returns a hash with n=1 for valid, existing args" do
				err_hash=@pt.update(@id, @raw_item, :collection=>@test_collection)
